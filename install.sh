...
if [ "`which sudo`" = "" ]; then
	...
	apt install -y python-pip python-tk git unzip libsm6 libxext6 tesseract-ocr python-opencv libsm6 libxext6 gcc unzip
else
	echo -e "Installing Python"
...
pip install pillow requests opencv-python keras tensorflow matplotlib pexpect pyocr Cython fuzzywuzzy[speedup] documentdbs
